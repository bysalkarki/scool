...
if (isset($_REQUEST['insert'])) {
	...
	if (!empty($name) && !empty($email) && !empty($pass)  && !empty($dob) && !empty($phone)) {
		if (preg_match('/^(97|98)\d{8}$/', $phone) && filter_var($email, FILTER_VALIDATE_EMAIL) && preg_match('/\s$/', $name)) {
			...
			$error = "<p class='alert alert-warning'>Please enter appropriate data</p>";
		}
	} else {
